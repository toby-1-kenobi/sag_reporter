...
describe Edit do
  ...
  let(:another_language) { FactoryBot.create(:language) }
  let(:curator) { FactoryBot.create(:user) }
  let(:edit_author) { FactoryBot.create(:user) }
  let(:language_edit) { FactoryBot.create(:edit, record_id: language.id, old_value: language.name, user: edit_author) }
  let(:pending_edit) { FactoryBot.create(:edit, record_id: language.id, status: Edit.statuses[:pending_single_approval], user: edit_author) }
  let(:double_pending_edit) { FactoryBot.create(:edit, record_id: another_language.id, status: Edit.statuses[:pending_double_approval], user: edit_author) }
  let(:national_pending_edit) { FactoryBot.create(:edit, record_id: language.id, status: Edit.statuses[:pending_national_approval], user: edit_author) }
  let(:approved_edit) { FactoryBot.create(:edit, record_id: language.id, status: Edit.statuses[:approved], user: edit_author) }
  let(:rejected_edit) { FactoryBot.create(:edit, record_id: language.id, status: Edit.statuses[:rejected], user: edit_author) }

  before do
    Curating.create(user: curator, geo_state: language.geo_states.first)
    edit_author.geo_states << language.geo_states.first
    edit_author.geo_states << another_language.geo_states.first
  end
  ...
  it 'sets the curated prompted date when prompting curators' do
    pending_edit.created_at = 8.days.ago
    pending_edit.save
    _(User.curating(pending_edit)).must_include curator
    Edit.prompt_curators
    curator.reload
