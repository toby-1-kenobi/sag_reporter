class MinistryOutput < ActiveRecord::Base
  ...
  def check_for_duplicates
    entry = self
    entries = MinistryOutput.where(month: entry.month, actual: entry.actual, church_ministry_id: entry.church_ministry_id, deliverable_id: entry.deliverable_id).order(updated_at: :desc)
    if entries.size > 0
